<?php
// =====================================================================
// FRONT END FORM SUBMMISION - SOURCE : https://webdevstudios.com/2015/03/30/use-cmb2-to-create-a-new-post-submission-form/
// =====================================================================
 ...
 */
function FineDiv_frontend_form_register()
{
  ...
  $cmb->add_field(array(
    'name' => __('New Post Title', 'FineDiv'),
  ...
  $cmb->add_field(array(
    'default_cb' => 'FineDiv_maybe_set_default_from_posted_values',
    'name' => __('New Post Content', 'FineDiv'),
  ...
  $cmb->add_field(array(
    'default_cb' => 'FineDiv_maybe_set_default_from_posted_values',
    'name' => __('Featured Image for New Post', 'FineDiv'),
  ...
  $cmb->add_field(array(
    'default_cb' => 'FineDiv_maybe_set_default_from_posted_values',
    'name' => __('Your Name', 'FineDiv'),
    'desc' => __('Please enter your name for author credit on the new post.', 'FineDiv'),
  ...
  $cmb->add_field(array(
    'default_cb' => 'FineDiv_maybe_set_default_from_posted_values',
    'name' => __('Your Email', 'FineDiv'),
    'desc' => __('Please enter your email so we can contact you if we use your post.', 'FineDiv'),
    'id' => 'submitted_author_email',
    'type' => 'text_email',
  ));

}
add_action('cmb2_init', 'FineDiv_frontend_form_register');



/**
 * Sets the front-end-post-form field values if form has already been submitted.
 * SOURCE : https://github.com/CMB2/CMB2-Snippet-Library/blob/master/front-end/cmb2-front-end-submit.php
 * @return string
 */
function FineDiv_maybe_set_default_from_posted_values($args, $field)
{
 ...
 * @return CMB2 object
 */
function FineDiv_frontend_cmb2_get()
{
	// Use ID of metabox in FineDiv_frontend_form_register
 ...
 */
function FineDiv_do_frontend_form_submission_shortcode($atts = array())
{

	// Get CMB2 metabox object
  $cmb = FineDiv_frontend_cmb2_get();
  ...
  if (($error = $cmb->prop('submission_error')) && is_wp_error($error)) {
    ...
    $output .= '<h3>' . sprintf(__('There was an error in the submission: %s', 'FineDiv'), '<strong>' . $error->get_error_message() . '</strong>') . '</h3>';
  }
  ...
  if (isset($_GET['post_submitted']) && ($post = get_post(absint($_GET['post_submitted'])))) {
    ...
    $output .= '<h3>' . sprintf(__('Thank you%s, your new post has been submitted and is pending review by a site administrator.', 'FineDiv'), esc_html($name)) . '</h3>';
  }

	// Get our form
  $output .= cmb2_get_metabox_form($cmb, 'fake-object-id', array('save_button' => __('Submit Post', 'FineDiv')));

  return $output;
}
add_shortcode('cmb_frontend_form', 'FineDiv_do_frontend_form_submission_shortcode');
...
function FineDiv_handle_frontend_new_post_form_submission()
{
  ...
  $img_id = FineDiv_frontend_form_photo_upload($new_submission_id, $post_data);
...
add_action('cmb2_after_init', 'FineDiv_handle_frontend_new_post_form_submission');
 ...
 */
function FineDiv_frontend_form_photo_upload($post_id, $attachment_post_data = array())
{
